...
export default function DisciplineArticlePage({
  ...
  advancedTopic?: { key: string; title: string }
}) {
  ...
  const libraryHref =
    advancedTopic && task
      ? disciplineAdvancedPath(discipline.slug, specialty.name, task.stage.key, advancedTopic.key)
      : disciplineLibraryPath(discipline.slug, specialty.name, task?.stage.key, task?.task.key)
  return (
    <div className="py-8 sm:py-12">
      <nav className="mb-6 text-sm text-slate-500" aria-label="面包屑">
        ...
        <span className="mx-2">/</span>
        <Link href={disciplineBeginnerPath(discipline.slug, specialty.name)}>{specialty.name}</Link>
        {task && (
          <>
            <span className="mx-2">/</span>
            <Link href={disciplineBeginnerPath(discipline.slug, specialty.name, task.stage.key)}>
              {task.stage.title}
            </Link>
            <span className="mx-2">/</span>
            {advancedTopic ? (
              <>
                <Link href={libraryHref}>进阶阅读</Link>
                <span className="mx-2">/</span>
                <Link href={libraryHref}>{advancedTopic.title}</Link>
              </>
            ) : (
      ...
      <div className="grid gap-10 lg:grid-cols-[15rem_minmax(0,1fr)] lg:items-start">
        <LearningSidebar
          ...
          selectedStage={task?.stage.key}
          selectedMode={advancedTopic ? 'advanced' : undefined}
          selectedTopic={advancedTopic?.key}
        />
        <article>
          <header
          ...
          <div className="mx-auto grid max-w-6xl gap-12 py-12 xl:grid-cols-[13rem_minmax(0,48rem)]">
            <aside className="hidden xl:block">
              ...
              >
                <summary className="mb-3 cursor-pointer text-xs font-black tracking-widest text-slate-500">
                  本文目录
                </summary>
